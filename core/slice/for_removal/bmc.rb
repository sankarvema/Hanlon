...
module ProjectHanlon
  class Slice
    # ProjectHanlon Slice Bmc
    # Used for all BMC/IPMI logic
    class Bmc < ProjectHanlon::Slice
      ...
      def bmc_help
        ...
        puts "    new BMCs with Hanlon.".red
        puts "BMC Commands:".yellow
        puts "\thanlon bmc [get] [all]                             " + "Display list of BMCs".yellow
        puts "\thanlon bmc [get] (UUID) [--query,-q (IPMI_QUERY)]  " + "Display info for a BMC".yellow
        puts "\thanlon bmc register (options...)                   " + "Registers a new BMC".yellow
        puts "\thanlon bmc update (UUID) --power-state,-p (STATE)  " + "Set power state using BMC".yellow
      ...
      def get_bmc_by_uuid
        ...
        bmc_uuid, options = parse_and_validate_options(option_items, "hanlon bmc [get] (UUID) [--query,-q IPMI_QUERY]", :require_all)
        includes_uuid = true if bmc_uuid
        matching_bmc = get_bmc_with_uuid(bmc_uuid)
        raise ProjectHanlon::Error::Slice::InvalidUUID, "no matching BMC (with a uuid value of '#{bmc_uuid}') found" unless matching_bmc
      ...
      #
      # @param [String] uuid
      # @return [ProjectHanlon::PowerControl::Bmc]
      ...
      def update_bmc_power_state
        ...
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        bmc_uuid, options = parse_and_validate_options(option_items, "hanlon bmc update UUID --power-state,-p NEW_STATE", :require_one)
        includes_uuid = true if bmc_uuid
      ...
      # This function is used to registers a BMC with Hanlon (or to change the values associated
      # with an existing BMC in the Hanlon database)
      def register_bmc
        ...
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        tmp, options = parse_and_validate_options(option_items, "hanlon bmc register (options...)", :require_all)
        ...
        begin
          ...
          # raise ProjectHanlon::Error::Slice::MissingArgument, "cannot register BMC without specifying the uuid, mac, and ip" unless bmc_uuid && mac_addr && ip_addr
          ...
          raise ProjectHanlon::Error::Slice::CouldNotRegisterBmc, "failed to refresh BMC state" unless new_bmc
          update_success = new_bmc.refresh_self
          raise ProjectHanlon::Error::Slice::InternalError, "failed to refresh BMC state" unless update_success
          slice_success(new_bmc.to_hash)
        rescue ProjectHanlon::Error::Slice::Generic => e
          raise e
        rescue StandardError => e
          raise ProjectHanlon::Error::Slice::InternalError, "error occurred while inserting BMC " + e.message
        end
      end
      ...
      def run_ipmi_query_cmd(bmc_uuid, sub_command, sub_command_action)
        ...
        begin
          ...
          raise ProjectHanlon::Error::Slice::InvalidUUID, "no matching BMC (with a uuid value of '#{bmc_uuid}') found" unless bmc
          ...
          raise ProjectHanlon::Error::Slice::InvalidCommand,
                "ipmi query command '#{ipmitool_cmd}' failed" unless command_success
        rescue ProjectHanlon::Error::Slice::Generic => e
          raise e
        rescue StandardError => e
          raise ProjectHanlon::Error::Slice::InternalError,
      ...
      def change_bmc_power_state(bmc_uuid, new_state)
        raise ProjectHanlon::Error::Slice::InvalidCommand, "missing details for command to change power state" unless
            new_state && new_state.length > 0
        begin
          raise ProjectHanlon::Error::Slice::MissingArgument, "missing the uuid value for command to change power state" unless bmc_uuid
          ...
          if new_state == "on"
            ...
            raise ProjectHanlon::Error::Slice::CommandFailed, result_string unless
            ...
            raise ProjectHanlon::Error::Slice::CommandFailed, "Unrecognized new_state '#{new_state}'; must be one of (on|off|cycle|reset)"
          end
          slice_success(result_string)
        rescue ProjectHanlon::Error::Slice::Generic => e
          raise e
        rescue StandardError => e
          raise ProjectHanlon::Error::Slice::InternalError, "an error occurred while changing power state -> #{e.message}"
        end
      end
      ...
      # ProjectHanlon::PowerControl::Bmc object to fill in correct values for the board_serial_number
      ...
      # caller
      #
      # @param [ProjectHanlon::PowerControl::Bmc] bmc
      # @param [Hash] bmc_hash
      ...
      def insert_bmc(bmc_hash)
        ...
        if bmc != nil
          ...
          bmc = ProjectHanlon::PowerControl::Bmc.new(bmc_hash)
          begin
            command_success, power_state = bmc.run_ipmi_query_cmd("power_status", @ipmi_username, @ipmi_password)
