...
LOGVIEWER_PREFIX = "ProjectHanlon::LogViewer::"
...
module ProjectHanlon
  class Slice
    ...
    # ProjectHanlon Slice Log
    # Used for log viewing
    class Log < ProjectHanlon::Slice
      ...
      # Initializes ProjectHanlon::Slice::Log including #slice_commands, #slice_commands_help
      # @param [Array] args
      def initialize(args)
        ...
        @logfile = File.join(get_logfile_path, "project_hanlon.log")
      end
      ...
      # uses the location of the Hanlon configuration file to determine the path to the
      ...
      def get_hanlon_log
        ...
        option_items = command_option_data(:get_hanlon_log)
        # parse and validate the options that were passed in as part of this
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        tmp, options = parse_and_validate_options(option_items, "hanlon log (options...)", :require_all)
        ...
        if filter_options.length > 0
          ...
          if num_lines_tail && filter_before_tail
            ...
            filter_then_tail_hanlon_log(filter_options, num_lines_tail)
            ...
            tail_then_filter_hanlon_log(filter_options, num_lines_tail)
          else
            # number of lines to tail was not included, so just filter
            filter_hanlon_log(filter_options)
          end
        elsif num_lines_tail
          # user passed in the number of lines to tail, but no filter options
          tail_hanlon_log(num_lines_tail)
        else
      ...
      private
      # Prints the contents from the current hanlon logfile to the command line
      def view_hanlon_log
        ...
        begin
          ...
          raise ProjectHanlon::Error::Slice::InternalError, "error while reading log file #{@logfile}; #{e.message}"
        end

      end

      # Prints the tail of the current hanlon logfile to the command line
      def tail_hanlon_log(num_lines_tail)
      ...
      # filters the current hanlon logfile, printing all matching lines
      def filter_hanlon_log(filter_options)
        ...
        begin
          ...
          raise ProjectHanlon::Error::Slice::InternalError, "error while filtering log file #{@logfile}; #{e.message}"
        end

      end

      # tails the current hanlon logfile, then filters the result
      def tail_then_filter_hanlon_log(filter_options, num_lines_tail)
        ...
        begin
          ...
          raise ProjectHanlon::Error::Slice::InternalError, "error while tailing, then filtering log file #{@logfile}; #{e.message}"
        end

      end

      # filters the current hanlon logfile, then tails the result
      def filter_then_tail_hanlon_log(filter_options, num_lines_tail)
        ...
        begin
          ...
          raise ProjectHanlon::Error::Slice::InternalError, "error while filtering, then tailing log file #{@logfile}; #{e.message}"
        end
        tail_of_file.each { |line|
          puts line
        }
