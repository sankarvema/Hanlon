...
require "error/error_dep"

module ProjectHanlon
  module Error

    ## This creates additional error classes with appropriate http_err code and log_severity.
    #def self.create_class(new_class, error_code, val={}, msg='', parent_str='ProjectHanlon::Error::Generic')
    ...
    #
    #  parent = ProjectHanlon::Error.contantize(parent_str)
    #  parent_module_str = parent_str.split('::')[0..-2].join('::')
    #  parent_module = ProjectHanlon::Error.contantize(parent_module_str)
    #
    ...
    [
        [ 'CannotCreatePolicyTable'                , 10 , {'@http_err' => :bad_request}            , ''          , 'ProjectHanlon::Error::Generic' ],
        [ 'MissingMultiCollectionOnGroupPersist'   , 31 , {'@http_err' => :internal_server_error}  , ''          , 'ProjectHanlon::Error::Generic' ],
    ].each do |err|
