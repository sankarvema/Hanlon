...
# Module used for all logging. Needs to be included in any ProjectOccam class that needs logging.
...
module ProjectOccam::Logging
  ...
  def logger
    ...
    @_logger ||= ProjectOccam::Logging.logger_for(classname, methodname)
  ...
  class << self
    ...
    def get_log_level
      if ENV['OCCAM_LOG_LEVEL'] == nil
        return 3
      end
      ENV['OCCAM_LOG_LEVEL'].to_i
    end
