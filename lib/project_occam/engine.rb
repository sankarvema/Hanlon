...
module ProjectOccam
  class Engine < ProjectOccam::Object
    ...
    def default_mk
      mk_images = []
      get_data.fetch_all_objects(:images).each { |i| mk_images << i if i.path_prefix == "mk" && i.verify(ProjectOccam.config.image_svc_path) == true }
    ...
    def mk_checkin(uuid, last_state)
      ...
      if node
        ...
        if (node.timestamp - old_timestamp) > ProjectOccam.config.register_timeout
    ...
    def checkin_action_override(uuid)
      checkin_file = "#{$occam_root}/conf/checkin_action.yaml"
    ...
    def default_mk_boot(uuid)
      ...
      default = ProjectOccam::PolicyTemplate::BootMK.new({})
    ...
    def node_status(node)
      ...
      max_active_elapsed_time = ProjectOccam.config.register_timeout
    ...
    def get_system_tags
      ...
      |tr|
        begin
          ProjectOccam::Tagging::TagRule.new(tr)
        rescue => e
    ...
    def remove_image(image)
      ...
      data = get_data
      unless image.remove(ProjectOccam.config.image_svc_path)
