...
module ProjectOccam
  module Tagging
    class TagRule < ProjectOccam::Object
        ...
        if key.class == String && value.class == String
          if compare == "equal" || compare == "like"
            if inverse.to_s == "true" || inverse.to_s == "false"
              ...
              tag_matcher = ProjectOccam::Tagging::TagMatcher.new({"@key" => key,
                                                                   "@value" => value,
                                                                   "@compare" => compare,
                                                                   "@inverse" => inverse.to_s},
                                                                  @uuid)
              if tag_matcher.class == ProjectOccam::Tagging::TagMatcher
      ...
      def tag_matcher_from_hash
        ...
        |tag_matcher_hash|
          if tag_matcher_hash.class == Hash || tag_matcher_hash.class == BSON::OrderedHash # change this to check descendant of Hash
            new_array << ProjectOccam::Tagging::TagMatcher.new(tag_matcher_hash, @uuid)
          else
      ...
      def from_hash(hash)
        ...
        |tag_matcher|
          if tag_matcher.class != ProjectOccam::Tagging::TagMatcher
            new_tag_matchers_array << ProjectOccam::Tagging::TagMatcher.new(tag_matcher, @uuid)
          else
      ...
      def tag_matcher_print
        ...
        if @tag_matchers.count > 0
          ...
          @tag_matchers.each do
          |tm|
            tm = ProjectOccam::Tagging::TagMatcher.new(tm, @uuid) unless tm.class == ProjectOccam::Tagging::TagMatcher
