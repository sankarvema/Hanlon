...
class ProjectOccam::Slice < ProjectOccam::Object
  ...
  # and initialize a constant pointing to the root of all Occam URIs
  OCCAM_URI_ROOT = SERVICE_CONFIG[:config][:swagger_ui][:base_path] + '/' +
  ...
  def initialize(args = nil)
    ...
    @prev_args = Stack.new
    @hidden = true
    @uri_root = ProjectOccam.config.mk_uri + ProjectOccam.config.websvc_root
    @data = get_data
  end
  ...
  def eval_command(command_hash)
    unless @command_array.count > 0
      ...
      if command_hash[:default]
        ...
        raise ProjectOccam::Error::Slice::Generic, "No Default Action"
      end
    end
    ...
    if command_hash[:else]
      ...
      raise ProjectOccam::Error::Slice::InvalidCommand, "System Error: no else action for slice"
    end
  end
  ...
  def slice_error(error, options = {})
    ...
    return_hash = {}
    log_level = :error
    if error.class.ancestors.include?(ProjectOccam::Error::Slice::Generic)
  ...
  def command_option_data(command)
    ...
    # cares, since nothing in Occam catches it specifically. --daniel 2013-04-16
  ...
  def get_options(options = { }, optparse_options = { })
    optparse_options[:banner] ||= "occam [command] [options...]"
  ...
  def validate_options(validate_options = { })
    ...
    case validate_options[:logic]
      when :require_one
        ...
        raise ProjectOccam::Error::Slice::MissingArgument, "Must provide one option from #{option_names.inspect}." if count < 1
      when :require_all
        validate_options[:option_items].each do
        |opt_item|
          raise ProjectOccam::Error::Slice::MissingArgument, "Must Provide: [#{opt_item[:description]}]" if opt_item[:required] && !validate_arg(validate_options[:options][opt_item[:name]])
        end
    end
  ...
  def print_command_help(command, option_items, contained_resource = nil)
    ...
    if contained_resource
      banner = ( option_items.select { |elem| elem[:uuid_is] == "required" }.length > 0 ?
          "occam #{slice_name} (#{slice_name.upcase}_UUID) #{contained_resource} #{command} (UUID) (options...)" :
          "occam #{slice_name} (#{slice_name.upcase}_UUID) #{contained_resource} #{command} (options...)")
    else
      banner = ( option_items.select { |elem| elem[:uuid_is] == "required" }.length > 0 ?
          "occam #{slice_name} #{command} (UUID) (options...)" :
          "occam #{slice_name} #{command} (options...)")
    end
  ...
  def throw_syntax_error
    command_str = "occam #{slice_name} #{@prev_args.join(" ")}"
