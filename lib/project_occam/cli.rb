...
class ProjectOccam::CLI
  ...
  def run(*argv)
    ...
    if @options[:nocolor] or !STDOUT.tty?
      ...
      # statements involving Strings in Occam)
    ...
    slice = argv.shift
    if call_occam_slice(slice, argv)
      return true
    end
  ...
  def call_occam_slice(raw_name, args)
    return nil if raw_name.nil?

    if raw_name == 'config' and @web_command and !@cli_private then
      @logger.error "Occam config called as web command"
      return false # Will yield 404 which is good. This slice doesn't exist in the web UI
    end

    name = file2const(raw_name)
    occam_module = Object.full_const_get(SLICE_PREFIX + name).new(args)
    occam_module.web_command = @web_command
    occam_module.verbose = @verbose
    occam_module.debug = @debug
    occam_module.slice_call
    return true
  rescue => e
    unless e.to_s =~ /uninitialized constant ProjectOccam::Slice::/
      @logger.error "Occam slice error: #{e.message}"
      print "\n [#{raw_name}] ".red
      print "<- #{e.message} \n".yellow
    end
    raise e if @debug
  ...
  def get_optparse
    OptionParser.new do |opts|
      opts.version   = ProjectOccam::VERSION
      opts.banner    = "#{opts.program_name} - #{opts.version}".green
      opts.separator "Usage: ".yellow
      opts.separator "    occam [slice name] [command argument] [command argument]...".red
      opts.separator ""
      ...
      opts.on_tail('-V', '--version', 'Display the version of Occam'.yellow) do
        puts opts.banner
        exit
      end
