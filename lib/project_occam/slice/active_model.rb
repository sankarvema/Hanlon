...
module ProjectOccam
  class Slice
    ...
    # ProjectOccam Slice Active_Model
    class ActiveModel < ProjectOccam::Slice

      def initialize(args)
        super(args)
        @hidden     = false
        @policies   = ProjectOccam::Policies.instance
        @uri_string = ProjectOccam.config.mk_uri + OCCAM_URI_ROOT + '/active_model'
      end
      ...
      def active_model_help
        ...
        puts "\toccam active_model [get] [all]          " + "View all active models".yellow
        puts "\toccam active_model [get] (UUID) [logs]  " + "View specific active model (log)".yellow
        puts "\toccam active_model logs                 " + "Prints an aggregate view of active model logs".yellow
        puts "\toccam active_model remove (UUID)|all    " + "Remove existing (or all) active model(s)".yellow
        puts "\toccam active_model --help|-h            " + "Display this screen".yellow
      end
      ...
      def get_logs
        ...
        # finally, based on the options selected, print the results
        lcl_slice_obj_ref = ProjectOccam::PolicyTemplate::Base.new({})
