...
module ProjectOccam
  class Slice
    # ProjectOccam Slice Bmc
    # Used for all BMC/IPMI logic
    class Bmc < ProjectOccam::Slice
      ...
      def bmc_help
        ...
        puts "    new BMCs with Occam.".red
        puts "BMC Commands:".yellow
        puts "\toccam bmc [get] [all]                             " + "Display list of BMCs".yellow
        puts "\toccam bmc [get] (UUID) [--query,-q (IPMI_QUERY)]  " + "Display info for a BMC".yellow
        puts "\toccam bmc register (options...)                   " + "Registers a new BMC".yellow
        puts "\toccam bmc update (UUID) --power-state,-p (STATE)  " + "Set power state using BMC".yellow
      ...
      def get_bmc_by_uuid
        ...
        bmc_uuid, options = parse_and_validate_options(option_items, "occam bmc [get] (UUID) [--query,-q IPMI_QUERY]", :require_all)
        includes_uuid = true if bmc_uuid
        matching_bmc = get_bmc_with_uuid(bmc_uuid)
        raise ProjectOccam::Error::Slice::InvalidUUID, "no matching BMC (with a uuid value of '#{bmc_uuid}') found" unless matching_bmc
      ...
      #
      # @param [String] uuid
      # @return [ProjectOccam::PowerControl::Bmc]
      ...
      def update_bmc_power_state
        ...
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        bmc_uuid, options = parse_and_validate_options(option_items, "occam bmc update UUID --power-state,-p NEW_STATE", :require_one)
        includes_uuid = true if bmc_uuid
      ...
      # This function is used to registers a BMC with Occam (or to change the values associated
      # with an existing BMC in the Occam database)
      def register_bmc
        ...
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        tmp, options = parse_and_validate_options(option_items, "occam bmc register (options...)", :require_all)
        ...
        begin
          ...
          # raise ProjectOccam::Error::Slice::MissingArgument, "cannot register BMC without specifying the uuid, mac, and ip" unless bmc_uuid && mac_addr && ip_addr
          ...
          raise ProjectOccam::Error::Slice::CouldNotRegisterBmc, "failed to refresh BMC state" unless new_bmc
          update_success = new_bmc.refresh_self
          raise ProjectOccam::Error::Slice::InternalError, "failed to refresh BMC state" unless update_success
          slice_success(new_bmc.to_hash)
        rescue ProjectOccam::Error::Slice::Generic => e
          raise e
        rescue StandardError => e
          raise ProjectOccam::Error::Slice::InternalError, "error occurred while inserting BMC " + e.message
        end
      end
      ...
      def run_ipmi_query_cmd(bmc_uuid, sub_command, sub_command_action)
        ...
        begin
          ...
          raise ProjectOccam::Error::Slice::InvalidUUID, "no matching BMC (with a uuid value of '#{bmc_uuid}') found" unless bmc
          ...
          raise ProjectOccam::Error::Slice::InvalidCommand,
                "ipmi query command '#{ipmitool_cmd}' failed" unless command_success
        rescue ProjectOccam::Error::Slice::Generic => e
          raise e
        rescue StandardError => e
          raise ProjectOccam::Error::Slice::InternalError,
      ...
      def change_bmc_power_state(bmc_uuid, new_state)
        raise ProjectOccam::Error::Slice::InvalidCommand, "missing details for command to change power state" unless
            new_state && new_state.length > 0
        begin
          raise ProjectOccam::Error::Slice::MissingArgument, "missing the uuid value for command to change power state" unless bmc_uuid
          ...
          if new_state == "on"
            ...
            raise ProjectOccam::Error::Slice::CommandFailed, result_string unless
            ...
            raise ProjectOccam::Error::Slice::CommandFailed, "Unrecognized new_state '#{new_state}'; must be one of (on|off|cycle|reset)"
          end
          slice_success(result_string)
        rescue ProjectOccam::Error::Slice::Generic => e
          raise e
        rescue StandardError => e
          raise ProjectOccam::Error::Slice::InternalError, "an error occurred while changing power state -> #{e.message}"
        end
      end
      ...
      # ProjectOccam::PowerControl::Bmc object to fill in correct values for the board_serial_number
      ...
      # caller
      #
      # @param [ProjectOccam::PowerControl::Bmc] bmc
      # @param [Hash] bmc_hash
      ...
      def insert_bmc(bmc_hash)
        ...
        if bmc != nil
          ...
          bmc = ProjectOccam::PowerControl::Bmc.new(bmc_hash)
          begin
            command_success, power_state = bmc.run_ipmi_query_cmd("power_status", @ipmi_username, @ipmi_password)
      ...
      def map_to_ipmitool_cmd(sub_command, sub_command_action)
        case "#{sub_command}, #{sub_command_action}"
          ...
          when "fru, print"
            ...
            raise ProjectOccam::Error::Slice::SliceCommandParsingFailed,
