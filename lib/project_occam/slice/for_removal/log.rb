...
LOGVIEWER_PREFIX = "ProjectOccam::LogViewer::"
...
module ProjectOccam
  class Slice
    ...
    # ProjectOccam Slice Log
    # Used for log viewing
    class Log < ProjectOccam::Slice
      ...
      # Initializes ProjectOccam::Slice::Log including #slice_commands, #slice_commands_help
      # @param [Array] args
      def initialize(args)
        ...
        @logfile = File.join(get_logfile_path, "project_occam.log")
      end
      ...
      # uses the location of the Occam configuration file to determine the path to the
      # ${OCCAM_HOME}/log directory (which is where the logfiles for Occam are located)
      ...
      def get_occam_log
        ...
        raise ProjectOccam::Error::Slice::NotImplemented,
        ...
        option_items = command_option_data(:get_occam_log)
        # parse and validate the options that were passed in as part of this
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        tmp, options = parse_and_validate_options(option_items, "occam log (options...)", :require_all)
        ...
        if filter_options.length > 0
          ...
          if num_lines_tail && filter_before_tail
            ...
            filter_then_tail_occam_log(filter_options, num_lines_tail)
      ...
      private
      # Prints the contents from the current occam logfile to the command line
      def view_occam_log
        ...
        begin
          ...
          raise ProjectOccam::Error::Slice::InternalError, "error while reading log file #{@logfile}; #{e.message}"
        end

      end

      # Prints the tail of the current occam logfile to the command line
      def tail_occam_log(num_lines_tail)
      ...
      # filters the current occam logfile, printing all matching lines
      def filter_occam_log(filter_options)
        ...
        begin
          ...
          raise ProjectOccam::Error::Slice::InternalError, "error while filtering log file #{@logfile}; #{e.message}"
        end

      end

      # tails the current occam logfile, then filters the result
      def tail_then_filter_occam_log(filter_options, num_lines_tail)
        ...
        begin
          ...
          raise ProjectOccam::Error::Slice::InternalError, "error while tailing, then filtering log file #{@logfile}; #{e.message}"
        end

      end

      # filters the current occam logfile, then tails the result
      def filter_then_tail_occam_log(filter_options, num_lines_tail)
        ...
        begin
          ...
          raise ProjectOccam::Error::Slice::InternalError, "error while filtering, then tailing log file #{@logfile}; #{e.message}"
        end
        tail_of_file.each { |line|
          puts line
        }
