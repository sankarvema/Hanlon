...
module ProjectOccam
  class Slice
    ...
    # ProjectOccam Slice Node (NEW)
    # Used for policy management
    class Node < ProjectOccam::Slice
      ...
      def initialize(args)
        ...
        @uri_string = ProjectOccam.config.mk_uri + OCCAM_URI_ROOT + '/node'
      ...
      def get_node_help
        return ["Node Slice: used to view the current list of nodes (or node details)".red,
                ...
                "\toccam node [get] [all]                      " + "Display list of nodes".yellow,
                "\toccam node [get] (UUID)                     " + "Display details for a node".yellow,
                "\toccam node [get] (UUID) [--field,-f FIELD]  " + "Display node's field values".yellow,
                "\toccam node --help                           " + "Display this screen".yellow,
      ...
      def get_node_by_uuid
        ...
        node_uuid, options = parse_and_validate_options(option_items, "occam node [get] (UUID) [--field,-f FIELD]", :require_all)
