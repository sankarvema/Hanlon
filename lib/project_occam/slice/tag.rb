...
module ProjectOccam
  class Slice
    ...
    # ProjectOccam Slice Tag
    # Used for managing the tagging system
    class Tag < ProjectOccam::Slice
      # Initializes ProjectOccam::Slice::Tag
      # @param [Array] args
      def initialize(args)
        super(args)
        @hidden = false
        @uri_string = ProjectOccam.config.mk_uri + OCCAM_URI_ROOT + '/tag'
      end
      ...
      def slice_commands
        ...
        # "occam tag matcher update --help" or "occam tag matcher add --help")
        commands[:matcher] = {}
      ...
      def tag_help
        if @prev_args.length > 1
          ...
          begin
            ...
            # the command occam tag (UUID) matcher update (or add), so need to split on the
      ...
      def get_tag_help
        return [ "Tag Slice:".red,
                 ...
                 "Tag commands:".yellow,
                 "\toccam tag [get] [all]                           " + "View Tag summary".yellow,
                 "\toccam tag [get] (UUID)                          " + "View details of a Tag".yellow,
                 "\toccam tag add (...)                             " + "Create a new Tag".yellow,
                 "\toccam tag update (UUID) (...)                   " + "Update an existing Tag ".yellow,
                 "\toccam tag remove (UUID)|all                     " + "Remove existing Tag(s)".yellow,
                 "Tag Matcher commands:".yellow,
                 "\toccam tag (T_UUID) matcher [get] [all]          " + "View Tag Matcher summary".yellow,
                 "\toccam tag (T_UUID) matcher [get] (UUID)         " + "View Tag Matcher details".yellow,
                 "\toccam tag (T_UUID) matcher add (...)            " + "Create a new Tag Matcher".yellow,
                 "\toccam tag (T_UUID) matcher update (UUID) (...)  " + "Update a Tag Matcher".yellow,
                 "\toccam tag (T_UUID) matcher remove (UUID)        " + "Remove a Tag Matcher".yellow,
                 "\toccam tag --help|-h                             " + "Display this screen".yellow].join("\n")
      end
      ...
      def add_tagrule
        ...
        tmp, options = parse_and_validate_options(option_items, "occam tag add (options...)", :require_all)
      ...
      def update_tagrule
        ...
        tagrule_uuid, options = parse_and_validate_options(option_items, "occam tag update (UUID) (options...)", :require_one)
      ...
      def remove_all_tagrules
        ...
        raise ProjectOccam::Error::Slice::MethodNotAllowed, "This method has been deprecated"
      end
      ...
      def add_matcher
        ...
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        tmp, options = parse_and_validate_options(option_items, "occam tag matcher add (options...)", :require_all)
      ...
      def update_matcher
        ...
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        matcher_uuid, options = parse_and_validate_options(option_items, "occam policy update UUID (options...)", :require_one)
