  ...
  class Slice
    ...
    class Model < ProjectOccam::Slice
      ...
      def initialize(args)
        super(args)
        @hidden = false
        @uri_string = ProjectOccam.config.mk_uri + OCCAM_URI_ROOT + '/model'
      end
      ...
      def get_model_help
        return ["Model Slice: used to add, view, update, and remove models.".red,
                ...
                "\toccam model [get] [all]                 " + "View all models".yellow,
                "\toccam model [get] (UUID)                " + "View specific model instance".yellow,
                "\toccam model [get] template|templates    " + "View list of model templates".yellow,
                "\toccam model add (options...)            " + "Create a new model instance".yellow,
                "\toccam model update (UUID) (options...)  " + "Update a specific model instance".yellow,
                "\toccam model remove (UUID)|all           " + "Remove existing model(s)".yellow,
                "\toccam model --help                      " + "Display this screen".yellow].join("\n")
      end
      ...
      def add_model
        ...
        tmp, options = parse_and_validate_options(option_items, "occam model add (options...)", :require_all)
      ...
      def update_model
        ...
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        model_uuid, options = parse_and_validate_options(option_items, "occam model update UUID (options...)", :require_one)
        ...
        if change_metadata
          raise ProjectOccam::Error::Slice::UserCancelled, "User cancelled Model creation" unless
              model.cli_create_metadata
        end
      ...
      def verify_image(model, image_uuid)
        uri = URI.parse ProjectOccam.config.mk_uri + OCCAM_URI_ROOT + "/image/#{image_uuid}"
        # and get the results of the appropriate RESTful request using that URI
        include_http_response = true
        result, response = rz_http_get(uri, include_http_response)
        if response.instance_of?(Net::HTTPBadRequest)
