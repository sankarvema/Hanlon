...
POLICY_PREFIX = "ProjectOccam::PolicyTemplate::"

# Root ProjectOccam namespace
module ProjectOccam
  class Slice

    # ProjectOccam Slice Policy (NEW))
    # Used for policy management
    class Policy < ProjectOccam::Slice

      # Initializes ProjectOccam::Slice::Policy including #slice_commands, #slice_commands_help
      # @param [Array] args
      def initialize(args)
        super(args)
        @hidden     = false
        @policies   = ProjectOccam::Policies.instance
        @uri_string = ProjectOccam.config.mk_uri + OCCAM_URI_ROOT + '/policy'
      end
      ...
      def get_policy_help
        return ["Policy Slice:".red,
                ...
                "\toccam policy [get] [all]                      " + "View all policies".yellow,
                "\toccam policy [get] (UUID)                     " + "View a specific policy".yellow,
                "\toccam policy [get] templates|types            " + "View available policy templates".yellow,
                "\toccam policy add (options...)                 " + "Create a new policy".yellow,
                "\toccam policy update (UUID) (options...)       " + "Update an existing policy".yellow,
                "\toccam policy remove (UUID)|all                " + "Remove existing policy(s)".yellow,
                "\toccam policy --help|-h                        " + "Display this screen".yellow].join("\n")
      end
      ...
      def add_policy
        ...
        tmp, options = parse_and_validate_options(option_items, "occam policy add (options...)", :require_all)
      ...
      def update_policy
        ...
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        policy_uuid, options = parse_and_validate_options(option_items, "occam policy update UUID (options...)", :require_one)
