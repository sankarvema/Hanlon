require "json"
require "project_occam/broker/base"

# Root ProjectOccam namespace
module ProjectOccam
  class Slice

    # ProjectOccam Slice Broker
    # Used for broker management
    class Broker < ProjectOccam::Slice

      # Root namespace for broker objects; used to find them
      # in object space in order to gather meta-data for creating brokers
      SLICE_BROKER_PREFIX = "ProjectOccam::BrokerPlugin::"

      # Initializes ProjectOccam::Slice::Broker including #slice_commands, #slice_commands_help
      # @param [Array] args
      def initialize(args)
        super(args)
        @hidden          = false
        @uri_string = ProjectOccam.config.mk_uri + OCCAM_URI_ROOT + '/broker'
      end
      ...
      def broker_help
        ...
        puts "\toccam broker [get] [all]                 " + "View all broker targets".yellow
        puts "\toccam broker [get] (UUID)                " + "View specific broker target".yellow
        puts "\toccam broker [get] plugin|plugins|t      " + "View list of available broker plugins".yellow
        puts "\toccam broker add (options...)            " + "Create a new broker target".yellow
        puts "\toccam broker update (UUID) (options...)  " + "Update a specific broker target".yellow
        puts "\toccam broker remove (UUID)|all           " + "Remove existing (or all) broker target(s)".yellow
        puts "\toccam broker --help|-h                   " + "Display this screen".yellow
      end
      ...
      def add_broker
        ...
        tmp, options = parse_and_validate_options(option_items, "occam broker add (options...)", :require_all)
      ...
      def update_broker
        ...
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        broker_uuid, options = parse_and_validate_options(option_items, "occam broker update (UUID) (options...)", :require_one)
        ...
        if change_metadata
          raise ProjectOccam::Error::Slice::UserCancelled, "User cancelled Broker creation" unless
              broker.cli_create_metadata
        end
      ...
      def remove_broker
        ...
        # subcommand (this method will return a UUID value, if present, and the
        # options map constructed from the @commmand_array)
        broker_uuid, options = parse_and_validate_options(option_items, "occam broker remove (UUID)|(--all)", :require_all)
        if !@web_command
        ...
        if options[:all]
          ...
          raise ProjectOccam::Error::Slice::MissingArgument, "Must provide a UUID for the broker to remove (or 'all' to remove all)"
        end
      end
