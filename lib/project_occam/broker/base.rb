...
module ProjectOccam
  module BrokerPlugin
    ...
    # Root namespace for Brokers defined in ProjectOccam for node hand off
    # @abstract
      ...
      def web_create_metadata(provided_metadata)
        ...
        |md|
          ...
          if provided_metadata[md]
            raise ProjectOccam::Error::Slice::InvalidBrokerMetadata, "Invalid Metadata [#{md.to_s}:'#{provided_metadata[md]}']" unless
                set_metadata_value(md_fld_name, provided_metadata[md], metadata[:validation])
          else
            if metadata[:default] != ""
              raise ProjectOccam::Error::Slice::MissingBrokerMetadata, "Missing metadata [#{md.to_s}]" unless
                  set_metadata_value(md_fld_name, metadata[:default], metadata[:validation])
            else
              raise ProjectOccam::Error::Slice::MissingBrokerMetadata, "Missing metadata [#{md.to_s}]" if metadata[:required]
            end
          end
        end
      end
