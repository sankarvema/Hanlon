...
module ProjectOccam::BrokerPlugin
  ...
  # Root namespace for Chef Broker plugin defined in ProjectOccam for node handoff
  class Chef < ProjectOccam::BrokerPlugin::Base
    ...
    BROKER_SUCCESS_MSG = "Occam Chef bootstrap completed."
    ...
    # any roles/recipes found in occam tags
    #
    # Occam tags that are Chef run_list entries will be of the form
    ...
    def run_list
      ...
      if @options[:metadata][:occam_tags]
        tag_string = @options[:metadata][:occam_tags]
    ...
    def occam_detail_data
      node = get_data.fetch_object_by_uuid(:node, @options[:uuid])
      occam_details = node.attributes_hash
      logger.debug "---------------occam details: #{occam_details}"
      occam_details
    end

    def first_boot
      {
        :occam_metadata => @options[:metadata],
        :occam_attributes => occam_detail_data,
        :run_list => run_list
      }
    end
