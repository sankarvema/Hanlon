...
# Root namespace for ProjectOccam
module ProjectOccam::BrokerPlugin
  # Root namespace for Script Broker plugin defined in ProjectOccam for node handoff.
  class Script < ProjectOccam::BrokerPlugin::Base
    # Script Error
    ...
    def initialize(hash)
      ...
      @req_metadata_hash = {
        ...
        "@script_path" => {
          :default      => "/tmp/occam_script",
          :example      => "/tmp/occam_script",
          :required     => false,
        ...
        "@data_path" => {
          :default      => "/tmp/occam_script_data",
          :example      => "/tmp/occam_script_data",
          :required     => false,
          :description  => "Script data path.",
          :validation   => '.*',
        },
        "@metadata_path" => {
          :default      => "/tmp/occam_script_metadata",
          :example      => "/tmp/occam_script_metadata",
          :required     => false,
          :description  => "Script metadata path.",
          :validation   => '.*',
        },
        "@log_path" => {
          :default      => "/tmp/occam_script.log",
          :example      => "/tmp/occam_script.log",
          :required     => false,
    ...
    def agent_hand_off(options = {})
      ...
      begin
        ...
        if @attempts < 3 then
          @attempts += 1
          logger.warn "Occam script broker connection timed out (attempt: #{@attempts} of 3): #{e}"
          sleep(30)
          retry
        else
          logger.error "Occam script broker connection timed out (no more attempts left): #{e}"
          return :broker_fail
        end
      rescue => e
        logger.error "Occam script broker error: #{e}"
        logger.error "Occam script broker output:\n---\n#{@output}\n---"
        return :broker_fail
      end

      logger.debug "Occam script broker output:\n---\n#{@output}\n---"
