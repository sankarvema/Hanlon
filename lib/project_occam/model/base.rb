...
module ProjectOccam
  module ModelTemplate
    # Root Model object
    # @abstract
    class Base < ProjectOccam::Object
      ...
      def node_metadata
        begin
          ...
          meta = {}
          logger.debug "Adding occam stuff"
          meta[:occam_tags] = @node.tags.join(',')
          meta[:occam_node_uuid] = @node.uuid
          meta[:occam_active_model_uuid] = @policy_uuid
          meta[:occam_model_uuid] = @uuid
          meta[:occam_model_name] = @name
          meta[:occam_model_description] = @description
          meta[:occam_model_template] = @template.to_s
          meta[:occam_policy_count] = @counter.to_s
      ...
      def image_svc_uri
        "http://#{config.image_svc_host}:#{config.image_svc_port}#{config.websvc_root}/image/#{@image_prefix}"
      end
      ...
      def web_create_metadata(provided_metadata)
        ...
        |md|
          ...
          if provided_metadata[md]
            raise ProjectOccam::Error::Slice::InvalidModelMetadata, "Invalid Metadata [#{md.to_s}:'#{provided_metadata[md]}']" unless
                set_metadata_value(md_fld_name, provided_metadata[md], metadata[:validation])
          else
            if metadata[:default] != ""
              raise ProjectOccam::Error::Slice::MissingModelMetadata, "Missing metadata [#{md.to_s}]" unless
                  set_metadata_value(md_fld_name, metadata[:default], metadata[:validation])
            else
              raise ProjectOccam::Error::Slice::MissingModelMetadata, "Missing metadata [#{md.to_s}]" if metadata[:required]
            end
          end
        end
      end
