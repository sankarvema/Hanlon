module ProjectOccam
  ...
  class Policies < ProjectOccam::Object
    include(ProjectOccam::Logging)
    include(Singleton)

    POLICY_PREFIX = "ProjectOccam::PolicyTemplate::"
    MODEL_PREFIX = "ProjectOccam::ModelTemplate::"
    ...
    class PolicyTable < ProjectOccam::Object
      ...
      def move_to_idx(policy_uuid, new_index)
        ...
        if new_index > (@p_table.count - 1) || new_index < 0
          raise ProjectOccam::Error::Slice::InputError, "New line number '#{new_index}' is not valid; should be an between 0 and #{@p_table.count - 1}"
        end
    ...
    def policy_table
      ...
      return pt if pt
      pt = ProjectOccam::Policies::PolicyTable.new({})
      pt = get_data.persist_object(pt)
      raise ProjectOccam::Error::CannotCreatePolicyTable, "Cannot create policy table" unless pt
      pt
    end
    ...
    def get_templates
      ProjectOccam::PolicyTemplate.class_children.map do |policy_template|
    ...
    def get_model_templates
      ProjectOccam::ModelTemplate.class_children.map do |policy_template|
