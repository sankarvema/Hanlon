...
module ProjectOccam
  module Persist
    # MongoDB version of {ProjectOccam::Persist::PluginInterface}
    # used by {ProjectOccam::Persist::Controller} when ':mongo' is the 'persist_mode' in
    # ProjectOccam configuration
    class MongoPlugin < PluginInterface
      ...
      def connect(hostname, port, username, password, timeout)
        ...
        @occam_database = @connection.db("project_occam")
        @connection.active?
      end
      ...
      # Checks whether DB 'ProjectOccam' is selected in MongoDB
      #
      # @return [Boolean] Connection status
      #
      def is_db_selected?
        #logger.debug "Is ProjectOccam DB selected?(#{(@occam_database != nil and @connection.active?)})"
        (@occam_database != nil and @connection.active?)
      end
      ...
      def collection_by_name(collection_name)
        if is_db_selected?
          @occam_database.collection(collection_name.to_s)
        else
