...
module ProjectOccam
  module Persist
    # Persistence Controller for ProjectOccam
    class Controller
      ...
      def initialize()
        ...
        if (config.persist_mode == :mongo)
          ...
          require "project_occam/persist/mongo_plugin" unless ProjectOccam::Persist.const_defined?(:MongoPlugin)
          @database = ProjectOccam::Persist::MongoPlugin.new
        elsif (config.persist_mode == :postgres)
          logger.debug "Using Postgres plugin"
          require "project_occam/persist/postgres_plugin" unless ProjectOccam::Persist.const_defined?(:PostgresPlugin)
          @database = ProjectOccam::Persist::PostgresPlugin.new
        elsif (config.persist_mode == :memory)
          logger.debug "Using in-memory plugin"
          require "project_occam/persist/memory_plugin" unless ProjectOccam::Persist.const_defined?(:MemoryPlugin)
          @database = ProjectOccam::Persist::MemoryPlugin.new
        else
      ...
      # Connect to database using ProjectOccam::Persist::Database::Plugin loaded
