...
module ProjectHanlon::BrokerPlugin
  ...
  # Root namespace for Chef Broker plugin defined in ProjectHanlon for node handoff
  class Chef < ProjectHanlon::BrokerPlugin::Base
    ...
    BROKER_SUCCESS_MSG = "Hanlon Chef bootstrap completed."
    ...
    # any roles/recipes found in hanlon tags
    #
    # Hanlon tags that are Chef run_list entries will be of the form
    ...
    def run_list
      ...
      if @options[:metadata][:hanlon_tags]
        tag_string = @options[:metadata][:hanlon_tags]
    ...
    def hanlon_detail_data
      node = get_data.fetch_object_by_uuid(:node, @options[:uuid])
      hanlon_details = node.attributes_hash
      logger.debug "---------------hanlon details: #{hanlon_details}"
      hanlon_details
    end

    def first_boot
      {
        :hanlon_metadata => @options[:metadata],
        :hanlon_attributes => hanlon_detail_data,
        :run_list => run_list
      }
    end
