...
module ProjectHanlon::BrokerPlugin
  # Root namespace for Script Broker plugin defined in ProjectHanlon for node handoff.
  class Script < ProjectHanlon::BrokerPlugin::Base
    # Script Error
    ...
    def initialize(hash)
      ...
      @req_metadata_hash = {
        ...
        "@script_path" => {
          :default      => "/tmp/hanlon_script",
          :example      => "/tmp/hanlon_script",
          :required     => false,
        ...
        "@data_path" => {
          :default      => "/tmp/hanlon_script_data",
          :example      => "/tmp/hanlon_script_data",
          :required     => false,
          :description  => "Script data path.",
          :validation   => '.*',
        },
        "@metadata_path" => {
          :default      => "/tmp/hanlon_script_metadata",
          :example      => "/tmp/hanlon_script_metadata",
          :required     => false,
          :description  => "Script metadata path.",
          :validation   => '.*',
        },
        "@log_path" => {
          :default      => "/tmp/hanlon_script.log",
          :example      => "/tmp/hanlon_script.log",
          :required     => false,
    ...
    def agent_hand_off(options = {})
      ...
      begin
        ...
        if @attempts < 3 then
          @attempts += 1
          logger.warn "Hanlon script broker connection timed out (attempt: #{@attempts} of 3): #{e}"
          sleep(30)
          retry
        else
          logger.error "Hanlon script broker connection timed out (no more attempts left): #{e}"
          return :broker_fail
        end
      rescue => e
        logger.error "Hanlon script broker error: #{e}"
        logger.error "Hanlon script broker output:\n---\n#{@output}\n---"
        return :broker_fail
      end

      logger.debug "Hanlon script broker output:\n---\n#{@output}\n---"
