  ...
  class Slice
    ...
    class Model < ProjectHanlon::Slice
      ...
      def initialize(args)
        super(args)
        @hidden = false
        @uri_string = ProjectHanlon.config.mk_uri + ProjectHanlon.config.websvc_root + '/model'
      end
      ...
      def verify_image(model, image_uuid)
        uri = URI.parse ProjectHanlon.config.mk_uri + ProjectHanlon.config.websvc_root + "/image/#{image_uuid}"
        # and get the results of the appropriate RESTful request using that URI
        include_http_response = true
        result, response = rz_http_get(uri, include_http_response)
        if response.instance_of?(Net::HTTPBadRequest)
