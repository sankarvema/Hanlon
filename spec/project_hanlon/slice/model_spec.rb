require 'spec_helper'
require 'project_hanlon/slice/model'

describe ProjectHanlon::Slice::Model do
  subject('slice') { ProjectHanlon::Slice::Model.new([]) }
  ...
  it "should be able to fetch all child template objects" do
    templates = slice.get_child_templates(ProjectHanlon::ModelTemplate)
    templates.should_not be_empty
    templates.each {|t| t.should be_a ProjectHanlon::ModelTemplate::Base }
  end
end
