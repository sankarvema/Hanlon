...
describe ProjectOccam::Config::Server do
  ...
  before :each do
    ...
    ProjectOccam::Config::Server._reset_instance
  end
  ...
  context "class" do
    subject { ProjectOccam::Config::Server }
    it { should respond_to "instance" }
  end

  describe ".instance" do
    it "should return the same instance if called twice" do
      ProjectOccam::Config::Server.instance.object_id.should ==
        ProjectOccam::Config::Server.instance.object_id
    end
    ...
    it "should load the configuration file doesn't exist" do
      ...
      ProjectOccam::Config::Server.instance.    # ...and britches
        should be_an_instance_of ProjectOccam::Config::Server
    end
    ...
    it "should upgrade an instance with a missing value" do
      ...
      Tempfile.open('this-config-file-has-missing-values') do |file|
        file.puts <<YAML
--- !ruby/object:ProjectOccam::Config::Server
...
YAML
        ...
        $config_server_path = file.path
        config = ProjectOccam::Config::Server.instance
  ...
  describe "defaults" do
    subject('defaults') { ProjectOccam::Config::Server.instance.defaults }
    ...
    it "should get the local IP for the default image_svc_host" do
      defaults['image_svc_host'].
        should == ProjectOccam::Config::Server.instance.get_an_ip
    end
  ...
  describe "save_as_yaml" do
    it "should silently skip if the file exists" do
      ...
      File.unlink(unexists.path) # whee
      $config_server_path = unexists.path
      ProjectOccam::Config::Server.instance.should be
      File.should exist unexists.path
      ...
      ProjectOccam::Config::Server.instance.save_as_yaml(unexists.path)
