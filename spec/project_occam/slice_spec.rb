...
describe ProjectOccam::Slice do
  ...
  describe "#command_option_data" do
    it "should raise an exception if the command is unknown" do
      expect {
        ProjectOccam::Slice::Node.new([]).command_option_data(:unknown_command)
      }.to raise_error
    end

    it "should return an array if the command is known" do
      ProjectOccam::Slice::Node.new([]).command_option_data(:get).
