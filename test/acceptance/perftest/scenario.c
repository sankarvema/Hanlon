...
static GOptionEntry options[] = {
  { "target", 0, 0, G_OPTION_ARG_STRING, &target,
    "The Hanlon server to run performance tests against", "HOST" },
  { "esxi-uuid", 0, 0, G_OPTION_ARG_STRING, &esxi_uuid,
    "The Hanlon ESXi OS image UUID", "UUID" },
  { "ubuntu-uuid", 0, 0, G_OPTION_ARG_STRING, &ubuntu_uuid,
    "The Hanlon Ubuntu OS image UUID", "UUID" },
  { "mk-uuid", 0, 0, G_OPTION_ARG_STRING, &mk_uuid,
    "The Hanlon MK image UUID", "UUID" },
...
TestSuite* test_suite_setup(int* argc, char*** argv) {
  GError*         error   = NULL;
  GOptionContext* context = g_option_context_new("- test Hanlon server performance");
