...
describe "ProjectHanlon::NodeJS::API" do
  ...
  describe ".RESTful Interface" do
    ...
    it "should bail on 404 when presented with URI encoded directory traversal" do
      uri = URI "http://127.0.0.1:#{config.api_port}#{ProjectHanlon.config.websvc_root}/api/..%2FGemfile"
      res = Net::HTTP.get_response(uri)
      res.code.should == '404'
    end

    it "should not do shell expansion and perform cat ~/.ssh/id_rsa" do
      uri = URI "http://127.0.0.1:#{config.api_port}#{ProjectHanlon.config.websvc_root}/api/-V/&&/cd/~/&&/cd/.ssh/&&/cat/id_rsa/;"
      res = Net::HTTP.get_response(uri)
      res.code.should == '404'
    end

    it "should bail on 404 when presented with binary zeroes" do
      uri = URI "http://127.0.0.1:#{config.api_port}#{ProjectHanlon.config.websvc_root}/api/-V/%00cd/~/&&/cd/.ssh/&&/cat/id_rsa/;"
      res = Net::HTTP.get_response(uri)
      res.code.should == '404'
    end

    it "should not do URI encoded shell expansion" do
      uri = URI "http://127.0.0.1:#{config.api_port}#{ProjectHanlon.config.websvc_root}/api/-V/%20%26%26%20cat%20%7E%2F.ssh%2Fid_rsa/;"
      res = Net::HTTP.get_response(uri)
      res.code.should == '404'
    end

    it "should bail on 404 when presented with -x" do
      uri = URI "http://127.0.0.1:#{config.api_port}#{ProjectHanlon.config.websvc_root}/api/-x"
      res = Net::HTTP.get_response(uri)
      res.code.should == '404'
    end

    it "should refuse to deliver the config slice" do
      uri = URI "http://127.0.0.1:#{config.api_port}#{ProjectHanlon.config.websvc_root}/api/config"
      res = Net::HTTP.get_response(uri)
      res.code.should == '404'
    end

    it "should refuse to combine web call with -j option" do
      uri = URI "http://127.0.0.1:#{config.api_port}#{ProjectHanlon.config.websvc_root}/api/-j/config"
      res = Net::HTTP.get_response(uri)
      res.code.should == '400'
    end
  end
