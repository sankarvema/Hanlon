...
// implement a JSON.lite_stringify serialization method
JSON.custom_stringify = function (obj, typeMap) {
    var t = typeof (obj);
    if (t != "object" || obj === null) {
        ...
        for (n in obj) {
            v = obj[n]; t = typeof(v);
            if (t == "string") v = '"'+v+'"';
            else if (t == "object" && v !== null) v = JSON.custom_stringify(v, typeMap[n]);
            if (typeMap[n] == "Hash") json.push((arr ? "" : '"' + n + '":') + String(v).slice(1, -1));
            else json.push((arr ? "" : '"' + n + '":') + String(v));
        }
...
(function() {
  ...
  HeaderView = (function(_super) {
    ...
    HeaderView.prototype.update = function(url, trigger) {
      if (trigger == null) {
        trigger = false;
      }
  ...
  OperationView = (function(_super) {
    ...
    OperationView.prototype.render = function() {
      ...
      this.model.path = this.model.path.replace(".json", "");
    ...
    OperationView.prototype.submitOperation = function(e) {
      var bodyParam, consumes, error_free, form, headerParams, invocationUrl, isFileUpload, isFormPost, map, match, o, obj, param, paramContentTypeField, responseContentTypeField, typeMap, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref5, _ref6, _ref7, _ref8, _ref9,
        _this = this;
      if (e != null) {
        e.preventDefault();
      }
      ...
      if (error_free) {
        ...
        if (isFileUpload) {
          ...
          for (_k = 0, _len2 = _ref7.length; _k < _len2; _k++) {
            ...
            if ((param.paramType === 'body' || 'form') && param.name !== 'file' && param.name !== 'File' && (map[param.name] != null)) {
              ...
              typeMap[param.name] = param.dataType;
            }
          }
          ...
          for (_l = 0, _len3 = _ref8.length; _l < _len3; _l++) {
            ...
            if (map[param.name] != null) {
              match = param.name.match(/^([^\[]+)\[([^\]]+)\](\[([^\]]+)\])?$/);
              if (match) {
                if (!bodyParam[match[1]]) {
                  bodyParam[match[1]] = {};
                  typeMap[match[1]] = {};
                }
                if (match[3]) {
                  if (!bodyParam[match[1]][match[2]]) {
                    bodyParam[match[1]][match[2]] = {};
                    typeMap[match[1]][match[2]] = {};
                  }
                  bodyParam[match[1]][match[2]][match[4]] = map[param.name];
                  typeMap[match[1]][match[2]][match[4]] = param.dataType;
                } else {
                  bodyParam[match[1]][match[2]] = map[param.name];
                  typeMap[match[1]][match[2]] = param.dataType;
                }
              } else {
        ...
        if (!isFileUpload) {
          bodyParam = JSON.custom_stringify(bodyParam, typeMap);
        }
    ...
    ParameterView.prototype.template = function() {
      if (this.model.isList) {
        ...
        if (this.options.readOnly) {
          ...
          if (this.model.required || this.model.paramType === 'path') {
